...
import { User, Phone, Calendar, Shield, MessageSquare, Link as LinkIcon, FileText, Settings, LogOut } from 'lucide-react';
import { Avatar, AvatarFallback } from '@/app/components/ui/avatar';
import { useNavigate } from 'react-router-dom';

export function Profile() {
  const navigate = useNavigate();
  const phoneNumber = localStorage.getItem('scamshield_phone') || '';

  const handleLogout = () => {
    localStorage.removeItem('scamshield_auth');
    localStorage.removeItem('scamshield_phone');
    navigate('/login');
  };
  ...
  const user = {
    ...
    avatar: phoneNumber ? phoneNumber.slice(-2) : 'SS',
  };
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="p-8 rounded-2xl bg-[#1a1f3a]/50 border border-[#00d9ff]/20 shadow-[0_0_30px_rgba(0,217,255,0.1)]">
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-bold text-white mb-4">ScamShield User</h2>
                  <div className="flex items-center justify-center gap-2 text-gray-400 mb-4">
                    <Phone className="w-4 h-4" />
                    <span className="text-sm font-mono">+91 {user.phone}</span>
                  </div>
                ...
                <button 
                  ...
                  className="w-full mt-6 px-6 py-3 bg-[#ff3b3b]/10 text-[#ff3b3b] border border-[#ff3b3b]/30 rounded-lg hover:bg-[#ff3b3b]/20 transition-all duration-300 hover:-translate-y-0.5 font-semibold flex items-center justify-center gap-2"
                >
